...
class MoT(nn.Module):
    def __init__(
        ...
        if "video" not in mixtures:
            raise ValueError("`mixtures` must include a 'video' expert.")
    ...
    def prefill_video_cache(
        ...
        if hasattr(expert, "reset_wam_adapter_cache"):
            expert.reset_wam_adapter_cache()
        x = video_tokens
        ...
        for layer_idx in range(self.num_layers):
            ...
            if hasattr(expert, "apply_wam_adapter"):
                x = expert.apply_wam_adapter(layer_idx=layer_idx, tokens=x)
    ...
    def forward(
        ...
        video_expert = self.mixtures["video"] if "video" in self.mixtures else None
        if video_expert is not None and hasattr(video_expert, "reset_wam_adapter_cache"):
            video_expert.reset_wam_adapter_cache()
        ...
        for layer_idx in range(self.num_layers):
            ...
            for name, seq_len in zip(self.expert_order, seq_lens):
                ...
                if name == "video" and hasattr(expert, "apply_wam_adapter"):
                    updated_tokens = expert.apply_wam_adapter(
                        layer_idx=layer_idx,
