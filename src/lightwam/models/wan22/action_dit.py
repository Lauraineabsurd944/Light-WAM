...
class AdapterConditionedActionHead(nn.Module):
    """Action head that conditions on pooled frozen/adapted video states."""

    def __init__(self, hidden_dim: int, fusion_dim: int, out_dim: int, eps: float):
        super().__init__()
        self.token_norm = nn.LayerNorm(hidden_dim, eps=eps, elementwise_affine=False)
        self.fusion_norm = nn.LayerNorm(fusion_dim, eps=eps)
    ...
    def forward(
        ...
        if fusion is None:
            fusion = torch.zeros(
                (tokens.shape[0], self.fusion_norm.normalized_shape[0]),
                dtype=tokens.dtype,
                device=tokens.device,
            )
        if fusion.ndim != 2 or fusion.shape[0] != tokens.shape[0]:
            raise ValueError(
                "Adapter-conditioned action fusion must be [B, D], "
                f"got shape {tuple(fusion.shape)} for batch {tokens.shape[0]}"
            )
        fusion_token = self.fusion_proj(self.fusion_norm(fusion)).unsqueeze(1).expand(
            -1,
            tokens.shape[1],
            -1,
        )
        return self.output(torch.cat([self.token_norm(tokens), fusion_token], dim=-1))
...
class ActionDiT(nn.Module):
    ...
    def __init__(
        ...
        self.video_hidden_dim = None if video_hidden_dim is None else int(video_hidden_dim)
        ...
        if self.use_wam_adapter:
            if self.video_hidden_dim is None:
                raise ValueError("`video_hidden_dim` is required when `use_wam_adapter=true`.")
            self.head = AdapterConditionedActionHead(
                hidden_dim=hidden_dim,
                fusion_dim=self.video_hidden_dim * 3,
                out_dim=action_dim,
                eps=eps,
            )
        else:
    ...
    def post_dit(self, tokens: torch.Tensor, pre_state: Dict[str, Any]) -> torch.Tensor:
        if self.use_wam_adapter:
            return self.head(tokens, pre_state.get("action_fusion"))
