...
from lightwam.utils.pytorch_utils import dict_apply
from lightwam.utils.logging_config import get_logger
from .base_processor import BaseProcessor

logger = get_logger(__name__)

class LightWAMProcessor(BaseProcessor):
    def __init__(
        self,
        # keys
    ...
    def _build_pixel_values_from_images_impl(
        self,
        data: Dict[str, Any],
        *,
        expected_num_obs_steps: int | None,
    ) -> torch.Tensor:
        processed_images = []
        for meta in self.shape_meta["images"]:
            ...
            if expected_num_obs_steps is not None:
                meta_shape = [expected_num_obs_steps] + shape
                assert image.shape == meta_shape, \
                    f"Expected shape {meta_shape}, got {image.shape} after transforms for key {key}"
            else:
                expected_channels, expected_h, expected_w = shape
                if tuple(image.shape[1:]) != (expected_channels, expected_h, expected_w):
                    raise ValueError(
                        f"Expected transformed image shape [T,{expected_channels},{expected_h},{expected_w}], "
        ...
        if self.num_output_cameras < pixel_values.shape[0]:
            logger.warning(
                "num_output_cameras %s is less than the number of cameras in data %s, "
                "truncating the input to the first %s cameras.",
    ...
    def build_pixel_values_from_images(self, data: Dict[str, Any]) -> torch.Tensor:
        """Apply the exact image preprocessing branch used by `preprocess()`.

        This is shared with latent-cache precompute so the encoded video input
        matches the training-time image pipeline under the same config.
        """
    ...
    def build_pixel_values_from_episode_images(self, data: Dict[str, Any]) -> torch.Tensor:
        ...
        Unlike `build_pixel_values_from_images`, this accepts arbitrary temporal
        length `T_episode` while preserving the exact per-frame image pipeline.
        """
        return self._build_pixel_values_from_images_impl(
            data,
            expected_num_obs_steps=None,
        )
    ...
    def preprocess_without_images(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Preprocess a sample when video frames are served from a latent cache.

        This keeps the original action/state/instruction path unchanged while
        skipping the image branch entirely.
        """
        sample = {}
        sample["instruction"] = self.augment_instruction(data)
        sample["image_is_pad"] = data["image_is_pad"]
...
class FastWAMProcessor(LightWAMProcessor):
    """Legacy Fast-WAM processor alias kept for backward compatibility."""
