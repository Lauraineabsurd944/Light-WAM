...
from lightwam.models.wan22.helpers.loader import _load_registered_model, _resolve_configs
from lightwam.models.wan22.wan_video_text_encoder import HuggingfaceTokenizer
...
def _collect_dataset_settings(data_cfg: DictConfig):
    ...
    for node_path, node in _iter_dataset_nodes(data_cfg, path="data"):
        ...
        text_embedding_model_id = node.get("text_embedding_model_id")
        if text_embedding_model_id not in (None, "", "null"):
            text_embedding_model_ids.add(str(text_embedding_model_id))

        logger.info("Discovered dataset node `%s` with %d dataset_dirs.", node_path, len(raw_dirs))

    return dataset_dirs, cache_dirs, context_lens, text_embedding_model_ids
...
def _resolve_text_embedding_model_id(
    text_embedding_model_ids: set[str],
    fallback_model_id: str,
) -> str:
    if len(text_embedding_model_ids) > 1:
        raise ValueError(
            "Found multiple text_embedding_model_id values in data config: "
            f"{sorted(text_embedding_model_ids)}. Please keep them consistent."
        )
    if len(text_embedding_model_ids) == 1:
        return next(iter(text_embedding_model_ids))
...
def _read_unique_prompts_from_hdf5_dirs(dataset_dirs: list[str]) -> list[str]:
    prompts: list[str] = []
    seen = set()
    ...
    logger.info(
        "Loaded %d HDF5 episode rows from %d datasets, deduplicated to %d prompts.",
        total_episode_rows,
        len(dataset_dirs),
        len(prompts),
    )
...
def main(cfg: DictConfig):
    ...
    dataset_dirs, cache_dirs, context_lens, text_embedding_model_ids = _collect_dataset_settings(cfg.data)
    if not cache_dirs:
    ...
    if override_prompt is not None:
        ...
        try:
            ...
            prompts = _read_unique_prompts_from_hdf5_dirs(dataset_dirs)
    if not prompts:
    ...
    text_embedding_model_id = _resolve_text_embedding_model_id(text_embedding_model_ids, model_id)
    enc_id = _model_id_to_enc_id(text_embedding_model_id)

    logger.info(
        "Preparing text encoder with text_embedding_model_id=%s tokenizer_model_id=%s device=%s dtype=%s context_len=%d overwrite=%s",
